...
def visualize_trajectory_2d(pose,landmarks=None,path_name="Unknown",show_ori=False):
    ...
    if landmarks is not None:
      ax.scatter(landmarks[0,:], landmarks[1,:], s=0.5)
...
def remove_landmark_outliers(landmarks):
    v = np.logical_and(np.logical_and(landmarks[0,:] > -1500, landmarks[0,:] < 1500), np.logical_and(landmarks[1,:] > -1500, landmarks[1,:] < 1500))
